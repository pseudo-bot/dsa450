...
export default function Cards({ topic, totalQs, remainingQs }) {
	return (
		<Link href={`/${topic}/problems`}>
			<div data-aos="fade-right" className="h-66 dark:bg-gray-800 dark:text-gray-300 bg-indigo-100 flex flex-col group justify-around p-5 w-full rounded-xl shadow-md my-4 sm:max-w-md sm:mx-8 relative cursor-pointer">
				<div className="text-3xl text-gray-700 dark:text-gray-300 uppercase font-bold m-2 mb-4">{topic}</div>
						<div className="m-2 sm:text-6xl text-5xl left-1/2 -translate-x-1/2 absolute font-extrabold text-indigo-200 dark:text-gray-600 z-0">{totalQs}</div>
